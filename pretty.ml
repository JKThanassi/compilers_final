...
let string_of_program_with (depth : int) (print_a : 'a -> string) (p : 'a program)
    : string
  =
  match p with
...
let rec string_of_aexpr_with (depth : int) (print_a : 'a -> string) (e : 'a aexpr)
    : string
  =
  ...
  then "..."
    ...
    | CSetItem (e, idx, newval, a) ->
      sprintf
        "%s[%s] := %s %s"
        (string_of_immexpr e)
        (string_of_immexpr idx)
        (string_of_immexpr newval)
        (print_a a)
    | CPrim1 (op, e, a) ->
    ...
    | CPrim2 (op, left, right, a) ->
      sprintf
        "(%s %s %s)%s"
        (string_of_immexpr left)
        (string_of_op2 op)
        (string_of_immexpr right)
        (print_a a)
    | CIf (cond, thn, els, _, a) ->
      sprintf
        "(if %s: %s else: %s)%s"
...
and string_of_aprogram_with (depth : int) (print_a : 'a -> string) (p : 'a aprogram)
    : string
  =
  match p with
...
let format_declgroup (fmt : Format.formatter) (print_a : 'a -> string) (d : 'a decl list)
    : unit
  =
  print_list
    fmt
...
let format_program (fmt : Format.formatter) (print_a : 'a -> string) (p : 'a program)
    : unit
  =
  match p with
  | Program (decls, body, a) ->
