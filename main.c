...
extern SNAKEVAL set_stack_bottom(uint64_t *stack_bottom) asm(
    "?set_stack_bottom");
...
SNAKEVAL equal(SNAKEVAL val1, SNAKEVAL val2) {
  ...
  if ((val1 & TUPLE_TAG_MASK) == TUPLE_TAG &&
    ...
    for (uint64_t i = 1; i <= tup1[0]; i++) {
      if (equal(tup1[i], tup2[i]) == BOOL_FALSE) return BOOL_FALSE;
    }
    return BOOL_TRUE;
  }
  return BOOL_FALSE;
...
void error(uint64_t code, SNAKEVAL val) {
  switch (code) {
    ...
    case ERR_SET_NOT_NUM:
      fprintf(stderr,
              "Error: tried to index into a tuple when setting with a value "
              "which is not a number: %ld, val: ",
              code);
      printHelp(stderr, val);
      break;
    case ERR_GET_NOT_NUM:
      fprintf(stderr,
              "Error: tried to index into a tuple when getting with a value "
              "which is not a number: %ld, val: ",
              code);
      printHelp(stderr, val);
      break;
    case ERR_VAL_NOT_STRING:
      fprintf(stderr,
              "Error: Expected a string value: %ld, instead got val: ", code);
      printHelp(stderr, val);
      break;
    case ERR_SUBSTRING_BAD_ARGS:
      fprintf(stderr,
              "Error: Substring requires that start/end > 0, start <= end, and "
              "end <= len of str: %ld",
              code);
      break;
    default:
...
uint64_t *try_gc(uint64_t *alloc_ptr, uint64_t bytes_needed,
  ...
  // TO_S, TO_E); naive_print_heap(FROM_S, FROM_E); printStack(BOOL_TRUE,
  // cur_stack_top, cur_frame, 0);

  // Abort early, if we can't allocate a new to-space
  if (new_heap == NULL) {
    fprintf(stderr,
            "Out of memory: could not allocate a new semispace for garbage "
            "collection\n");
    fflush(stderr);
