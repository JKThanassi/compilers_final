...
uint64_t *copy_if_needed(uint64_t *garter_val_addr, uint64_t *heap_top)
{
  ...
  {
    uint64_t *heap_thing_addr = (uint64_t *)(garter_val - (masked_garter_val == TUPLE_TAG ? TUPLE_TAG : CLOSURE_TAG));
    ...
    if ((*heap_thing_addr & TUPLE_TAG_MASK) == FORWARDING_TAG)
    {
      ...
      uint64_t tagged_fwd = untagged_fwd + (masked_garter_val == TUPLE_TAG ? TUPLE_TAG : CLOSURE_TAG);
    ...
    {
      // tuple
      len = *heap_thing_addr + (*heap_thing_addr % 2 == 0 ? 2 : 1); // account for len val and padding
    ...
    {
      uint64_t num_vars = heap_thing_addr[2]; // number of closed over variables is the third word in the closure tuple
    ...
    for (int i = 0; i < len; i++)
    {
      ...
      // heap_thing_addr[i] = *heap_top; // I think we need to set the value here ?
    }
  }
