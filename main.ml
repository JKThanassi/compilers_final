...
let set_strategy s =
  ...
  | _ ->
    raise
      (Arg.Bad
         (sprintf "'%s' is not an allocation strategy, use either naive or register" s))
;;
...
let () =
  ...
  Arg.parse
    ...
    (fun name ->
      if !filename_set
      then raise (Arg.Bad "Cannot compile more than one file name")
      else (
        filename_set := true;
        filename := name))
    "Compiler options:";
  ...
  | Error (errs, trace) ->
    if !show_trace
    then eprintf "%s%s" (ExtString.String.join sep (print_trace trace)) sep
    else ();
    eprintf "Errors:\n";
